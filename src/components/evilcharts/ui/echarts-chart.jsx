...
function buildChartCss(id, config) {
  const colorConfig = Object.entries(config).filter(([, item]) => item.colors);
  if (!colorConfig.length) return "";
  const varsFor = (theme) => colorConfig.flatMap(([key, item]) => {
  ...
  return Object.entries(THEMES).map(([theme, prefix]) => `${prefix} [data-chart=${id}] {
${varsFor(theme)}
}`).join("\n");
}
...
function withAlpha(color, alpha) {
  ...
  const base = a === void 0 ? 1 : Number.parseFloat(a) || 0;
  return `rgba(${r}, ${g}, ${b}, ${(base * alpha).toFixed(3)})`;
}
function resolveColors(container, config, seriesKeys) {
...
function indicatorBackground(key, colorsCount) {
...
function flattenColor(color, base) {
  const parse = (value) => value.match(/rgba?\(([^)]+)\)/)?.[1].split(",").map((part) => Number.parseFloat(part)) ?? [0, 0, 0, 1];
  const [r, g, b, a = 1] = parse(color);
  const [baseR, baseG, baseB] = parse(base);
  const mix = (channel, baseChannel) => Math.round(channel * a + baseChannel * (1 - a));
