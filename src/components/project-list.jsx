...
export function ProjectList({ projects, compact = false }) {
  ...
  return (
    <section className="space-y-5">
      ...
      <div className={cn("transition-opacity duration-100", !viewReady && "opacity-0", view === "grid" && "grid gap-8 sm:grid-cols-2")}>
        {shown.map((project, index) => (
          <article key={project.id} className={cn("group", view === "list" && "flat-row")}>
            {view === "grid" ? (
              <div className="portfolio-hover space-y-3">
                <a href={project.deployedUrl || project.githubUrl} target="_blank" rel="noreferrer" className="relative block aspect-[16/10] overflow-hidden rounded-lg bg-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-clay-500">
                  <Image src={project.imageUrl} alt={`${project.title} product preview`} fill priority={index < 2} sizes="(max-width: 640px) 100vw, 320px" className="object-cover object-top transition-transform duration-200 [@media(hover:hover)_and_(pointer:fine)]:group-hover:scale-[1.015] motion-reduce:transform-none motion-reduce:transition-none" />
                </a>
                <ProjectCopy project={project} />
              </div>
            ) : <div className="portfolio-hover"><ProjectCopy project={project} /><div className="pointer-events-none absolute top-1/2 left-[calc(100%+2rem)] hidden w-64 -translate-y-1/2 overflow-hidden rounded-lg border border-border bg-background opacity-0 shadow-lg transition-opacity duration-150 motion-reduce:transition-none [@media(hover:hover)_and_(pointer:fine)]:group-hover:opacity-100 group-focus-within:opacity-100 xl:block"><div className="relative aspect-[16/10]"><Image src={project.imageUrl} alt="" fill sizes="256px" className="object-cover object-top" /></div></div></div>}
          </article>
        ))}
      </div>
    </section>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-medium tracking-tight"><a href={project.deployedUrl || project.githubUrl} target="_blank" rel="noreferrer" className="after:absolute after:inset-0 after:rounded-md">{project.title}</a></h2>
      ...
      <div className="relative z-10 flex w-fit flex-wrap gap-2 pt-1" aria-label={`${project.title} technology stack`}>
...
function ProjectLink({ href, label, children }) {
  return <a href={href} target="_blank" rel="noreferrer" aria-label={label} className="relative z-10 grid size-8 place-items-center rounded-md text-muted-foreground transition-colors duration-100 hover:bg-muted hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-clay-500 [&_svg]:size-3.5 [&_svg]:stroke-[1.75]">{children}</a>;
}
